...
object foldmap {
  object sequential {
//    def foldMap[A, B: Monoid](it: Iterable[A])(f: A => B = identity[A]): B = it.foldLeft(Monoid[B].empty)(_ |+| f(_))
//    def foldMap[A, B: Monoid](it: Iterable[A])(f: A => B): B = it.foldLeft(Monoid[B].empty)(_ |+| f(_))
  ...
  object parallel {
//    def foldMap[A, B: Monoid](it: Iterable[A])(f: A => B = identity[A]): Future[B] = {
  ...
  object monadic {
//    def foldMap[A, M[_]: Monad, B: Monoid](it: Iterable[A])(f: A => M[B] = _.pure[Id]): M[B] = it.foldLeft(Monoid[B].empty.pure[M]){ (m, a) =>
